...
zoomLevel = 4  # when zoomLevel is positive it zooms in, when its negative it zooms out
...
while True:
    ...
    ret2, corners = cv2.findChessboardCorners(gray, chess_shape, None)

    # If found, do the processing
    if ret2 and processing:
        # pts2 is used for defining the perspective transform
        # c1 = find_left_top_corner(corners)
        pts2 = np.float32([corners[0, 0], corners[chess_shape[0] - 1, 0],
                           corners[len(corners) - 1, 0], corners[len(corners) - chess_shape[0], 0]])
